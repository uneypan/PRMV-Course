...
from models.LPRNet_History import *
...
def detect(save_img=False):
    ...
    if classify:
        modelc = init_rec_model(device,opt.rec_weights)
    ...
    print('Done. (%.3fs, avg. %.1ffps)' % (time.time() - t0, dataset.nframes/(time.time() - t0)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rec-weights', nargs='+', type=str, default='weights/crnn_small.pth', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='demo/example.mp4', help='source')  # file/folder, 0 for webcam
