...
import time
import argparse
from datasets.alphabets import plate_chr
...
def init_rec_model(device,model_path):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='weights/crnn_small.pth', help='model.pt path(s)')  
    parser.add_argument('--image_path', type=str, default='demo/license', help='source') 
    ...
    model = init_rec_model(device,opt.model_path)
        ...
        for pic_ in file_list:
            # try:
                ...
                if img.shape[-1]!=3:
                    img = cv2.cvtColor(img,cv2.COLOR_BGRA2BGR)
                plate=get_plate_result(img,device,model,img_size)
                plate_ori = pic_.split('/')[-1].split('_')[0].split('.')[0]
